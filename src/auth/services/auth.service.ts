...
export class AuthService {
  ...
  async login(user: any) { // Agora login recebe um usuário validado
  ...
  async generateRefreshToken(userId: string) {
    ...
    const refreshToken = this.refreshTokenRepository.create({ token, user: { id: userId }, expiresAt });
  ...
  async refreshToken(refreshTokenDto: RefreshTokenDto) {
    const storedToken = await this.refreshTokenRepository.findOne({ where: { token: refreshTokenDto.refreshToken } });
